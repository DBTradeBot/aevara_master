...
const _devToolsOn = bool.fromEnvironment('AEVARA_DEV_TOOLS', defaultValue: true);
...
class DevNavigatorFab extends StatelessWidget{
  ...
  Widget build(BuildContext c){
    ...
    return FloatingActionButton(onPressed: ()=>_open(c), child: const Icon(Icons.bug_report));
  }
  void _open(BuildContext c){
    showModalBottomSheet(context:c, isScrollControlled:true, builder:(_)=>Padding(
      padding: const EdgeInsets.fromLTRB(16,12,16,24), child:_DevNavigatorSheet(routes:_routes)));
  }
}
...
class _DevNavigatorSheet extends StatefulWidget{
  ...
  @override State<_DevNavigatorSheet> createState()=>_DevNavigatorSheetState();
}
class _DevNavigatorSheetState extends State<_DevNavigatorSheet>{
  String q='';
  @override Widget build(BuildContext c){
    final filtered = widget.routes.where((m)=>m['name']!.toLowerCase().contains(q.toLowerCase())).toList();
    return SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children:[
      ...
      Row(children:[
        ...
        FilledButton.tonal(onPressed: ()=>ThemeController.mode.value=ThemeMode.dark, child: const Text('Dark')),
        const SizedBox(width:8),
        FilledButton.tonal(onPressed: ()=>ThemeController.mode.value=ThemeMode.system, child: const Text('System')),
      ]),
      const SizedBox(height:12),
      Flexible(child: ListView.builder(shrinkWrap:true, itemCount:filtered.length, itemBuilder:(_,i){
        final r=filtered[i];
        return ListTile(title: Text(r['name']!), subtitle: Text(r['route']!), onTap:(){ Navigator.pop(context); Navigator.pushNamed(context, r['route']!); });
      }))
    ]));
  }
}
