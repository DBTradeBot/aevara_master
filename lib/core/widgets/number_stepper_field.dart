...
class _NumberStepperFieldState extends State<NumberStepperField> {
  ...
  void _delta(double d) {
    final v = (double.tryParse(_c.text) ?? widget.min) + d;
    final clamped = v.clamp(widget.min, widget.max);
    _c.text = clamped.toStringAsFixed(0);
  ...
  Widget build(BuildContext c) => Row(children: [
        ...
        IconButton.filledTonal(
            onPressed: () => _delta(-widget.step),
            icon: const Icon(Icons.remove)),
        const SizedBox(width: 4),
        IconButton.filled(
            onPressed: () => _delta(widget.step), icon: const Icon(Icons.add)),
      ]);
}
