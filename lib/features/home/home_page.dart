...
class HomePage extends StatelessWidget{ const HomePage({super.key});
  @override Widget build(BuildContext c)=>AppShell(currentIndex:0, title: 'Home', body: ListView(padding: const EdgeInsets.all(16), children:[
    ...
    const Row(children:[ Expanded(child: Card(child: ListTile(title: Text('Sleep'), subtitle: Text('7h 42m'), trailing: Text('+0.3h')))), SizedBox(width:8),
      Expanded(child: Card(child: ListTile(title: Text('HRV'), subtitle: Text('64 ms'), trailing: Text('+4')))), SizedBox(width:8),
      Expanded(child: Card(child: ListTile(title: Text('Steps'), subtitle: Text('8,410'), trailing: Text('-2%')))), ]),
