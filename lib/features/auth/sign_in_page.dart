              ...
              children: [
                Row(
                  children: [
                    ...
                    Text('brand.ai', style: Theme.of(context).textTheme.labelLarge),
                  ],
                ),
                ...
                const SizedBox(height: 16),
                AevButton.primary(_loading ? 'Signing in...' : 'Sign in', onPressed: _loading ? null : () async {
                  if (!_formKey.currentState!.validate()) return;
                  setState(() => _loading = true);
                  await Future.delayed(const Duration(milliseconds: 500));
                  if (mounted) {
                    Navigator.of(context).pushReplacementNamed('/onboarding/intro');
                  }
                }),
                const SizedBox(height: 12),
                Row(children: const [
                ...
                const SizedBox(height: 12),
                AevButton.secondary('Continue with Google', leading: const Icon(Icons.g_mobiledata), onPressed: (){
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('OAuth stubbed.')));
                }),
                const SizedBox(height: 12),
                Row(
                  children: [
