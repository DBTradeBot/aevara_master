...
class _CommunityFeedPageState extends State<CommunityFeedPage>{ int cheers=0;
  @override Widget build(BuildContext c)=>Scaffold(appBar: AppBar(title: const Text('Community Feed')), body: ListView(children:[
    FeedItemTile(title:'@alex earned a Sleep Streak badge', subtitle:'7 nights in a row', onCheer:(){setState(()=>cheers++); snack(c,'Cheered! ($cheers)');}),
    const Divider(),
    FeedItemTile(title:'@sam joined 10% Steps challenge', subtitle:'Let\'s go!', onCheer:(){setState(()=>cheers++); snack(c,'Cheered! ($cheers)');}),
    const Divider(),
    FeedItemTile(title:'@jules completed +30 min Sleep experiment', subtitle:'Great job', onCheer:(){setState(()=>cheers++); snack(c,'Cheered! ($cheers)');}),
  ]));
}
